...
class ASFOnlineSuperpositionManager:
    FORMAT_VERSION = 1
    METHOD = "binary_sign_binding_delta_cache"

    def __init__(self, base_state, base_model_path, modules, base_scene, active_scene, seed, output_dir):
    ...
    def from_bundle(cls, bundle_path, active_scene=None, output_dir=None):
        ...
        if payload.get("method") != cls.METHOD:
            raise RuntimeError(
                f'Unsupported superposition bundle method: {payload.get("method")} != {cls.METHOD}'
            )
        manager = cls(
    ...
    def expected_init_state(self, scene_name):
        ...
        if scene_name in self.scene_order:
            return self.materialize_full_state(scene_name, exact=True)
        return self.materialize_full_state(self.base_scene, exact=True)
    ...
    def ensure_scene(self, scene_name):
        ...
        if scene_name not in self.scene_deltas:
    ...
    def recover_scene_delta(self, scene_name, exact=True):
        ...
        self.ensure_scene(scene_name)
        if exact and scene_name in self.scene_deltas:
            return OrderedDict(
    ...
    def export(self, tag, step_idx, update_idx, score=None, best_score=None, source_checkpoint=None, materialize_scene_names=None):
        self.rebuild_storage()
        ...
        for scene_name in unique_scenes:
            scene_state = self.materialize_full_state(scene_name, exact=True)
