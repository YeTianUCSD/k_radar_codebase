'''
Unsupervised online adaptation runner for K-Radar ASF.

The stream split is consumed once, in dataset order. A frozen/EMA teacher
generates conservative pseudo boxes for every incoming batch, and the student
is updated with pseudo-label detection loss plus teacher/student consistency.
Periodic full evaluation runs on the fixed test split from the same config.
'''
...
def parse_args():
    parser = argparse.ArgumentParser(description='ASF unsupervised online adaptation')
    ...
    parser.add_argument('--pseudo_conf_thr', type=float, default=0.8,
                        help='Minimum teacher score used to accept pseudo boxes.')
    parser.add_argument('--pseudo_max_boxes', type=int, default=30,
                        help='Maximum pseudo boxes kept per sample after score filtering. <=0 keeps all.')
    parser.add_argument('--min_pseudo_boxes', type=int, default=1,
                        help='Minimum pseudo boxes required to enable pseudo detection loss for a batch.')
    parser.add_argument('--skip_empty_pseudo', action='store_true',
                        help='Skip optimizer step when no pseudo boxes are accepted and consistency loss is disabled.')
    parser.add_argument('--consistency_weight', type=float, default=0.1,
                        help='Weight for anchor-level teacher/student classification consistency.')
    parser.add_argument('--consistency_conf_thr', type=float, default=0.3,
                        help='Only teacher cls logits above this sigmoid confidence contribute to consistency.')
    parser.add_argument('--ema_decay', type=float, default=0.999,
                        help='EMA decay used to update teacher from student after each optimizer step.')
...
def make_empty_gt_boxes(batch_size):
    return torch.zeros((int(batch_size), 1, 8), dtype=torch.float32)


def replace_gt_boxes(batch, gt_boxes):
    batch['gt_boxes'] = gt_boxes
    batch['num_objs'] = [int((gt_boxes[idx, :, 7] > 0).sum().item()) for idx in range(gt_boxes.shape[0])]
    return batch


def boxes_in_roi(boxes, roi_xyz):
    if boxes.numel() == 0:
        return torch.zeros((0,), dtype=torch.bool, device=boxes.device)
    x_min, y_min, z_min, x_max, y_max, z_max = [float(v) for v in roi_xyz]
    return (
        (boxes[:, 0] >= x_min) & (boxes[:, 0] <= x_max) &
        (boxes[:, 1] >= y_min) & (boxes[:, 1] <= y_max) &
        (boxes[:, 2] >= z_min) & (boxes[:, 2] <= z_max) &
        (boxes[:, 3] > 0.1) & (boxes[:, 4] > 0.1) & (boxes[:, 5] > 0.1)
    )


def pseudo_gt_from_teacher(pred_dicts, batch_size, roi_xyz, args):
    list_gt = []
    counts = []
    max_boxes = 1
    for batch_idx in range(int(batch_size)):
        pred = pred_dicts[batch_idx]
        boxes = pred['pred_boxes'].detach().cpu()
        scores = pred['pred_scores'].detach().cpu()
        labels = pred['pred_labels'].detach().cpu().float()

        keep = scores >= float(args.pseudo_conf_thr)
        keep = keep & boxes_in_roi(boxes, roi_xyz).cpu()
        if keep.any():
            boxes = boxes[keep]
            scores = scores[keep]
            labels = labels[keep]
            if int(args.pseudo_max_boxes) > 0 and boxes.shape[0] > int(args.pseudo_max_boxes):
                top_idx = torch.argsort(scores, descending=True)[:int(args.pseudo_max_boxes)]
                boxes = boxes[top_idx]
                labels = labels[top_idx]
            gt = torch.cat([boxes[:, :7], labels.view(-1, 1)], dim=1).float()
        else:
            gt = torch.zeros((0, 8), dtype=torch.float32)
        list_gt.append(gt)
        counts.append(int(gt.shape[0]))
        max_boxes = max(max_boxes, int(gt.shape[0]))

    padded = torch.zeros((int(batch_size), max_boxes, 8), dtype=torch.float32)
    for batch_idx, gt in enumerate(list_gt):
        if gt.shape[0] > 0:
            padded[batch_idx, :gt.shape[0], :] = gt
    return padded, counts


def teacher_student_cls_consistency(student_logits, teacher_logits, conf_thr):
    if student_logits is None or teacher_logits is None:
        return None
    teacher_probs = torch.sigmoid(teacher_logits.detach())
    mask = teacher_probs >= float(conf_thr)
    if not torch.any(mask):
        return student_logits.new_tensor(0.0)
    return F.binary_cross_entropy_with_logits(student_logits[mask], teacher_probs[mask])
...
def append_metric_row(path_csv, row):
    fieldnames = [
        'event', 'timestamp', 'step_idx', 'update_idx',
        'loss', 'loss_det', 'loss_cons', 'num_pseudo', 'grad_norm',
...
def write_run_meta(path_log, args, runtime_cfg, trainable_info, missing, unexpected):
    meta = {
        ...
        'pseudo_conf_thr': args.pseudo_conf_thr,
        'pseudo_max_boxes': args.pseudo_max_boxes,
        'min_pseudo_boxes': args.min_pseudo_boxes,
        'skip_empty_pseudo': args.skip_empty_pseudo,
        'consistency_weight': args.consistency_weight,
        'consistency_conf_thr': args.consistency_conf_thr,
...
def main():
    ...
    print(f"* Pseudo conf thr = {args.pseudo_conf_thr}", flush=True)
    print(f"* Consistency weight = {args.consistency_weight}", flush=True)
    ...
    from utils.util_pipeline import build_network
    teacher_network = build_network(pline).cuda()
    missing_teacher, unexpected_teacher = load_model_checkpoint(teacher_network, args.init_model)
    teacher_network = freeze_teacher(teacher_network)
    print(f'* Teacher loaded. Missing keys: {len(missing_teacher)} / Unexpected keys: {len(unexpected_teacher)}')
    ...
    for batch in pbar:
        ...
        teacher_batch = replace_gt_boxes(clone_batch(batch), make_empty_gt_boxes(batch['batch_size']))
        with torch.no_grad():
            teacher_network.eval()
            teacher_out = teacher_network(teacher_batch)
        teacher_logits = teacher_network.head.forward_ret_dict.get('cls_preds', None)
        pseudo_gt_boxes, pseudo_counts = pseudo_gt_from_teacher(
            teacher_out['pred_dicts'],
            batch['batch_size'],
            pline.cfg.DATASET.roi.xyz,
            args,
        )
        num_pseudo = int(sum(pseudo_counts))

        student_batch = replace_gt_boxes(clone_batch(batch), pseudo_gt_boxes)
        pline.network.train()
        pline.network.training = True

        optimizer.zero_grad(set_to_none=True)
        dict_net = pline.network(student_batch)
        student_logits = pline.network.head.forward_ret_dict.get('cls_preds', None)

        loss_det = dict_net['fused_feat'].new_tensor(0.0)
        if num_pseudo >= int(args.min_pseudo_boxes):
            if pline.get_loss_from == 'head':
                loss_det = pline.network.head.loss(dict_net)
            elif pline.get_loss_from == 'detector':
                loss_det = pline.network.loss(dict_net)
            else:
                raise RuntimeError(f'Unsupported get_loss_from={pline.get_loss_from}')

        loss_cons = teacher_student_cls_consistency(
            student_logits,
            teacher_logits,
            args.consistency_conf_thr,
        )
        if loss_cons is None:
            loss_cons = loss_det.new_tensor(0.0)

        loss = loss_det + float(args.consistency_weight) * loss_cons
        if (
            num_pseudo < int(args.min_pseudo_boxes) and
            float(args.consistency_weight) <= 0.0 and
            args.skip_empty_pseudo
        ):
            clear_batch(batch)
            clear_batch(teacher_batch)
            clear_batch(student_batch)
            continue
        ...
        optimizer.step()
        update_ema_teacher(teacher_network, pline.network, args.ema_decay)
        ...
        if pline.is_logging and 'logging' in dict_net:
            ...
            pline.log_train_iter.add_scalar('online/loss_det', float(loss_det.detach().cpu().item()), idx_log_iter)
            pline.log_train_iter.add_scalar('online/loss_cons', float(loss_cons.detach().cpu().item()), idx_log_iter)
            pline.log_train_iter.add_scalar('online/num_pseudo', num_pseudo, idx_log_iter)
        ...
        append_metric_row(metrics_csv, {
            ...
            'update_idx': update_idx,
            'loss': loss_value,
            'loss_det': float(loss_det.detach().cpu().item()),
        ...
        pbar.set_postfix(
            ...
            pseudo=num_pseudo,
            best='nan' if best_score is None else f'{best_score:.4f}',
        )
        clear_batch(batch)
        clear_batch(teacher_batch)
        clear_batch(student_batch)
