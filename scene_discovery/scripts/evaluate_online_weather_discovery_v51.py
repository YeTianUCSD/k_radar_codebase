#!/usr/bin/env python3
"""Evaluate adaptive Seq1-only open-world online weather discovery and reuse."""
...
from scene_discovery.adaptive_weather_context import AdaptiveWeatherContextManager  # noqa: E402
...
from scene_discovery.online_protocol import (  # noqa: E402
    concatenate_partition_stream, normalize_sequence,
)
...
from scene_discovery.weather_protocol import (  # noqa: E402
    annotate_weather_stream, build_weather_orders, sequence_weather_mapping,
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=REPOSITORY_ROOT / "results/SceneDiscovery/OnlineWeatherDiscoveryV51",
    )
    ...
    parser.add_argument("--change-persistence", type=int, default=4)
    parser.add_argument("--candidate-min-windows", type=int, default=8)
    parser.add_argument("--candidate-max-windows", type=int, default=24)
    parser.add_argument("--provisional-windows", type=int, default=4)
    parser.add_argument("--cooldown-windows", type=int, default=12)
    parser.add_argument("--reference-windows", type=int, default=32)
    parser.add_argument("--recent-windows", type=int, default=6)
    parser.add_argument("--distribution-threshold-ratio", type=float, default=0.6)
    parser.add_argument("--strong-departure-ratio", type=float, default=1.5)
    ...
    parser.add_argument("--min-radius-ratio", type=float, default=0.8)
    parser.add_argument("--max-radius-ratio", type=float, default=2.0)
    parser.add_argument("--boundary-tolerance", type=int, default=15)
...
def save_timeline(frame, decisions, true_events, predicted_events, path, title):
    labels = sorted(frame["stream_true_sequence"].astype(str).unique())
    ...
    axes[0].set_ylabel("True weather\n(scoring only)")
...
def main():
    ...
    run_dir = make_run_dir(args.output_root.expanduser().resolve(), "online_weather_v51")
    indices = {split: bank.index(split) for split in ("train", "test")}
    weather_by_sequence = sequence_weather_mapping(
        pd.concat([indices["train"], indices["test"]], ignore_index=True)
    )
    initial_weather = weather_by_sequence[initial_scene]
    ...
    orders = build_weather_orders(
        scene_order, initial_scene, weather_by_sequence,
    ...
    for order_id, blocks in orders:
        ...
        for multiplier, change_threshold, match_ratio, match_threshold in threshold_settings:
            manager = AdaptiveWeatherContextManager(
                change_threshold=change_threshold,
                match_threshold_ratio=match_ratio,
                change_persistence=args.change_persistence,
                candidate_min_windows=args.candidate_min_windows,
                candidate_max_windows=args.candidate_max_windows,
                provisional_windows=args.provisional_windows,
                cooldown_windows=args.cooldown_windows,
                reference_windows=args.reference_windows,
                recent_windows=args.recent_windows,
                distribution_threshold_ratio=args.distribution_threshold_ratio,
                strong_departure_ratio=args.strong_departure_ratio,
                coverage_threshold=args.coverage_threshold,
                match_margin=args.switch_margin,
            ...
            weather_index = annotate_weather_stream(window_index, weather_by_sequence)
            truth = weather_index["stream_true_sequence"].astype(str).to_numpy()
            metrics, true_events, predicted_events = evaluate_causal_events(
                weather_index, decisions, {initial_weather: initial_context},
                args.boundary_tolerance, len(manager.memories),
            )
            false_creates = int(
                (predicted_events["is_false_boundary"]
                 & predicted_events["action"].eq("create")).sum()
            ) if len(predicted_events) else 0
            metrics.update({
                "false_create_events": false_creates,
                "false_creates_per_1000_windows": 1000.0 * false_creates / len(window_index),
                "provisional_entries": manager.provisional_entries,
                "provisional_cancellations": manager.provisional_cancellations,
                "provisional_cancellation_rate": (
                    manager.provisional_cancellations / manager.provisional_entries
                    if manager.provisional_entries else float("nan")
                ),
                "created_contexts": len(manager.memories) - 1,
            })
            ...
            decision_frame = weather_index.copy()
            ...
            decision_frame["mapped_weather"] = mapped
            decision_frame["mapped_weather_correct"] = (
                decision_frame["mapped_weather"].astype(str)
                == decision_frame["stream_true_weather"].astype(str)
            )
            ...
            for output, collection in (
                ...
                if collection is event_frames:
                    output["weather"] = output["sequence"].astype(str)
            ...
            if order_id == "canonical" and is_deployment:
                canonical_outputs = (weather_index, decisions, true_events, predicted_events)
    ...
    events_frame = pd.concat(event_frames, ignore_index=True)
    predicted_frame = pd.concat(predicted_event_frames, ignore_index=True)
    decisions_frame.to_csv(run_dir / "weather_decisions.csv", index=False)
    events_frame.to_csv(run_dir / "true_weather_event_results.csv", index=False)
    predicted_frame.to_csv(run_dir / "predicted_weather_events.csv", index=False)
    group_keys = ["order_id", "change_multiplier", "match_threshold_ratio", "weather"]
    per_weather_windows = (
        decisions_frame.groupby(
            [
                "order_id", "change_multiplier", "match_threshold_ratio",
                "stream_true_weather",
            ],
            as_index=False,
        )
        .agg(
            windows=("mapped_weather_correct", "size"),
            mapped_weather_accuracy=("mapped_weather_correct", "mean"),
        )
        .rename(columns={"stream_true_weather": "weather"})
    )
    per_weather_events = events_frame.groupby(group_keys, as_index=False).agg(
        transition_events=("true_event_id", "size"),
        detected_transitions=("matched_boundary", "sum"),
        exact_successes=("exact_event_success", "sum"),
        registry_collisions=("registry_collision", "sum"),
    )
    per_weather_windows.merge(
        per_weather_events, on=group_keys, how="left"
    ).fillna({
        "transition_events": 0,
        "detected_transitions": 0,
        "exact_successes": 0,
        "registry_collisions": 0,
    }).to_csv(run_dir / "per_weather_metrics.csv", index=False)
    if canonical_outputs is None:
        raise RuntimeError("canonical deployment output was not produced")
    save_timeline(
        *canonical_outputs,
        run_dir / "canonical_weather_deployment_timeline.png",
        (
            "V5.1 adaptive open-world online weather discovery, "
            f"change={deployment_threshold:.4f}, match_ratio={deployment_match_ratio:.2f}"
        ),
    )
    atomic_write_json(run_dir / "run_meta.json", {
        "schema_version": "kradar-online-weather-discovery/v5.1",
        "status": "complete",
        "created_utc": utc_now(),
        "descriptor_bank": str(bank.root),
        "arguments": vars(args),
        "manager_inputs": (
            "projected descriptors only; no sequence, weather, road, lighting, "
            "block, split, visit, or boundary metadata"
        ),
        "decision_policy": (
            "dual-timescale departure -> stable candidate collection -> coverage-based "
            "REUSE or provisional candidate -> delayed CREATE -> cooldown"
        ),
        "evaluation_policy": (
            "weather labels and weather-change boundaries are attached only after "
            "inference for causal one-to-one event matching and strict CREATE/REUSE scoring"
        ),
        "weather_by_sequence_scoring_only": weather_by_sequence,
        "calibration": calibration_payload,
        "orders": order_payload,
        "run_dir": str(run_dir),
    })
    print("V5.1 Seq1-only weather calibration:")
    print(pd.DataFrame([calibration_payload]).to_string(index=False))
    print("\nV5.1 adaptive open-world online weather metrics across orders:")
    display = [
        "change_multiplier", "change_threshold", "match_threshold_ratio",
        ...
        "context_count_error", "false_creates_per_1000_windows",
        "provisional_cancellation_rate", "window_accuracy", "ari", "nmi",
    ]
