...
class A2Fusion(nn.Module):
    def __init__(self, model_cfg, grid_size, **kwargs):
        ...
        for idx_key, temp_key in enumerate(self.key_feats):
            if to_embed_method == 'None':
                ...
                if to_embed_is_layer_norm:
                    ...
                    setattr(
                        self,
                        f'to_embed_{temp_key}',
                        nn.Conv2d(dim_feats[idx_key], dim_common, kernel_size=1, bias=False),
                    )
            ...
            if ucp_method is None:
                setattr(self, f'to_patch_embed_{temp_key}', nn.Identity())
            elif ucp_method == 'Linear1':
                if ucp_is_layer_norm:
                    setattr(self, f'to_patch_embed_{temp_key}', nn.Sequential(nn.LayerNorm(dim_patch_input), nn.Linear(dim_patch_input, dim_patch, bias=False), nn.LayerNorm(dim_patch)))
                else:
        ...
        self.fuser = nn.MultiheadAttention(dim_patch, n_heads, 0.0, batch_first=True)
        ...
        if pft_method == 'None':
            ...
            if pft_is_layer_norm:
                self.pft = nn.Sequential(nn.LayerNorm(dim_patch_input), nn.Linear(dim_patch, dim_patch, bias=False), nn.LayerNorm(dim_patch))
            else:
